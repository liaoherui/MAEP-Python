...
for s in snl:
	bq[s]={}
for filename in os.listdir('Tabel_'+ero):
	fb=open('Tabel_'+ero+'/'+filename,'r')
	line=fb.readline()
...
def bar(bq,outdir):
	...
	plt.xticks(index+barwidth/count_qua,class_name,rotation=13)
	plt.ylabel('number  of  bins',fontsize=12)
	#plt.legend(loc='upper right')
	plt.legend(loc='center left', bbox_to_anchor=(0.3, 1.08),ncol=4)
	...
	for m in mul_qua_arr:
		...
		count_bar_stack+=1
	plt.xticks(index,class_name,rotation=13)
	plt.ylabel(' proportion  of  bins',fontsize=12)
	#plt.legend(loc='upper right')
	plt.legend(loc='center left', bbox_to_anchor=(0.3, 1.08),ncol=4)
...
def tax_abundance(bq,output):
	...
	for key1 in genus:
		for key2 in genus[key1]:
			if key2 not in uni_genus:
				...
				uni_genus[key2]+=1
	#color related
        color_bar=['#fc5c65','#fd9644','#fed330','#26de81','#2bcbba','#45aaf2','#4b7bec','#a55eea','#d1d8e0','#778ca3','#2C3A47','#B33771']
	#Genus stack bar
	...
	for m in mul_gen_arr:
                ...
                count_bar_stack+=1
	plt.xticks(index,class_name,rotation=13)
        plt.ylabel(' proportion  of  genus',fontsize=12)
        plt.legend(loc='center left', bbox_to_anchor=(0.0, 1.08),ncol=5)
